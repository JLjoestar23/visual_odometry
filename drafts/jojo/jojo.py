...
def main():
    ...
    for i, img in enumerate(img_list[1:1551], start=1):
        if img.lower().endswith(".png"):
            print(f"Processing Image #: {i}")
            
            # process the next frame
            next_frame = cv.imread(f"{data_dir}/image_0/{img}", 
                                   cv.IMREAD_GRAYSCALE)

            # LK optical flow to track keypoints into the next frame
            prev_kp, next_kp = keypoint_tracking(prev_frame, next_frame, 
                                                 prev_kp)

            E, mask = cv.findEssentialMat(next_kp, prev_kp, K, cv.RANSAC, 
...
def plot_2d_trajectory(poses):
    ...
    x = positions[0:1550, 0]
    z = positions[0:1550, 2]
